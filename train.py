...
def compute_gradient_penalty(model, real_images, fake_images, eps):
    ...
    convex_images = eps * real_images + (1 - eps) * fake_images
    convex_images.requires_grad_(True)

    preds = model.discriminate(convex_images)
    preds_ = torch.ones_like(preds)
    grad_ = autograd.grad(preds, convex_images, grad_outputs=preds_,
                          create_graph=True, retain_graph=True)[0]
    
    grad_ = grad_.view(batch_size, -1)
    grad_ = grad_.norm(2, dim=1)
    gp_loss = ((grad_ - 1) ** 2).mean()
    # ========================================================
...
def train_step_wgan(batch, model, optimizer, gp):
    ...
    gen_loss = preds[:batch_size].mean()
    disc_loss = preds[batch_size:].mean()
